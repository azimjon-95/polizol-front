...
import { useGetAllEmployeesSalaryInfoQuery } from "../../context/alarApi";
...
function AttendanceHistory() {
  ...
  const { data: employeesData } = useGetAllEmployeesSalaryInfoQuery({
  ...
  const unitOptions = useMemo(() => {
    ...
    const units = new Set(
      mergedData.map((record) => record.unit).filter(Boolean)
    );
  ...
  if (isLoading)
    return (
      <div className="xsd-loading-container">
        <Spin size="large" />
      </div>
    );
  ...
  return (
    <div className="xsd-attendance-history-container">
      <div className="xsd-attendance-header no-print">
        <span className="xsd-subject-label">
          ...
          <table className="xsd-attendance-table">
            <thead>
              <tr className="xsd-table-header-row">
                <th className="xsd-table-header-cell xsd-student-number-header">
                  №
                </th>
                <th className="xsd-table-header-cell xsd-student-name-header">
                  To'liq ism
                </th>
                {dateHeaders.map((date, index) => (
                  <th
                    key={index}
                    className="xsd-table-header-cell xsd-date-header"
                  >
                    <div className="xsd-date-header-content">
                      <div className="xsd-date-day">{date.format("DD")}</div>
                      <div className="xsd-date-weekday">
                        {weekdayMap[date.format("dd")]}
                      </div>
                    </div>
                  </th>
                ))}
                <th className="xsd-table-header-cell xsd-stats-header">
                  Jami smena
                </th>
                {showPrintButton && (
                  <th className="xsd-table-header-cell xsd-stats-header">
              ...
              {processedData.map((employee, index) => (
                <tr key={employee.id} className="xsd-table-body-row">
                  ...
                  {dateHeaders.map((date, dateIndex) => {
                    const attendance =
                      employee.attendance[date.format("YYYY-MM-DD")];
                    return (
                      <td
                        key={dateIndex}
                        ...
                        <SignaturePad
                          ...
                          canvasProps={{
                            ...
                            style: {
                              // yumaloqlash // fon
                              cursor:
                                "url('https://cdn-icons-png.flaticon.com/512/1827/1827951.png') 0 24, auto",
                            },
                          }}
                        />
                        // </td>
