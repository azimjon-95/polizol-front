          ...
          <div className="invoice-delivery-form">
            ...
            {deliveryItems.map((item, index) => (
              <div key={index} className="invoice-delivery-item">
                <label>
                  {role !== "direktor" && item.remainingQuantity > 0 && (
                    <input
                      type="checkbox"
                ...
                <div>
                  Qoldiq: {(item.remainingQuantity || 0).toLocaleString()}{" "}
                  {item.size || "dona"}
                </div>
                {item.selected && (
                  <input
                    ...
                    placeholder={`Yuborish miqdori: ${item.remainingQuantity}`}
                    onChange={(e) => {
                      const val = e.target.value;
                      if (
                        val === "" ||
                        (parseInt(val) >= 1 && parseInt(val) <= item.remainingQuantity)
                      ) {
                        handleDeliveryItemChange(
                          index,
                          "deliveryQuantity",
...
//   const handleGroupChange = (group) => {
//     if (deliveredGroups.includes(group)) {
//       setDeliveredGroups((prev) =>
//         prev.includes(group) ? prev.filter((g) => g !== group) : [...prev, group]
//       );
//     } else {
//       setDeliveredGroups([...deliveredGroups, group]);
//     }
//   };
...
//   return (
//     <div>
//       <Modal className="modaldiliver-box"
//         open={modalState.isDeliveryModalOpen}
//         onCancel={() => {
//           closeModal();
//         }}
...
//                   </div>
//                   <div>
//                     Yuborilgan: {item.deliveredQuantity.toLocaleString()}{" "}
//                     {item.size || "dona"}
//                   </div>
//                   <div>
//                     Qoldiq: {item.remainingQuantity.toLocaleString()} {item.size || "dona"}
//                   </div>
//                   {item.selected && (
//                     <input
//                       type="number"
