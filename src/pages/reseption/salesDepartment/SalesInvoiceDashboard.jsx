...
import { setSearchQuery, clearSearchQuery } from '../../../context/actions/authSearch'; // Adjust path to your searchSlice
...
const SalesInvoiceDashboard = () => {
    ...
    // Redux hooks to access search state and dispatch actions
    const dispatch = useDispatch();
    const { searchQuery } = useSelector((state) => state.search);

    // Filter salesData based on searchQuery
    const filteredSalesData = useMemo(() => {
        if (!searchQuery) return salesData; // Return all data if no search query
        return salesData.filter((customer) =>
    ...
    const totalDebtAmount = filteredSalesData?.filter((i) => i.balansStatus === "Qarzdor")?.reduce((sum, sale) => sum + (sale.balans || 0), 0);
    const totalHaqdorAmount = filteredSalesData?.filter((i) => i.balansStatus === "Haqdor")?.reduce((sum, sale) => sum + (sale.balans || 0), 0);
                ...
                <table className="hip-table" aria-label="Sales Invoice Table">
                    ...
                    <tbody>
                        {isLoading ? (
                            ...
                            filteredSalesData.map((customer) => {
