...
const SalesInvoiceDashboard = () => {
    ...
    // Fetch ONLY first page (recent + first old page) and stats
    const { data, isLoading: initialLoading, refetch } = useGetFilteredSalesQuery({
        page: 1,
        limit: PAGE_LIMIT,
    ...
    // -------------------- SCROLL ANCHOR (pagination click bo'lsa joy saqlansin) --------------------
    // Pagination wrapper ref (faqat 1 marta render qiling!)
    ...
    const restoreAnchor = useCallback((snap) => {
        if (!snap) return;
        const el = paginationRef.current;
        if (!el) return;
        const rect = el.getBoundingClientRect();
        const delta = rect.top - snap.top;
    ...
    // IMPORTANT:
    // - Searchni backendga yuboryapmiz
    // - Frontendda yana filter qilmang (aks holda total count va pagination chalkashadi)
    const filteredSalesData = useMemo(() => allSalesData, [allSalesData]);

    const hasData = useMemo(() => (recentSales.length + oldSales.length) > 0, [recentSales.length, oldSales.length]);

    // -------------------- INITIAL/RESET LOGIC --------------------
    // muammo: pagination click bo'lganda ham "loader butun table" chiqib, DOM almashtirib scrollni buzardi
    // muammo: data (page=1) refetch bo'lsa oldSales qayta set bo'lib append yo'qolardi
    //
    // yechim:
    // - faqat search o'zgarganda (yoki birinchi load bo'lganda) state reset qilamiz
    ...
    useEffect(() => {
        ...
        if (data?.innerData) {
            const shouldReset = (!hasData) || (prevSearch !== currentSearch);
            ...
            if (shouldReset) {
                ...
                // append bo'lgan listni buzmasdan statsni yangilab qo'yamiz
                setTotalOldCount(data.innerData.totalOldSalesCount || 0);
            }
        }

        prevSearchRef.current = currentSearch;
    }, [data, searchQuery, hasData]);

    // -------------------- LOAD MORE (APPEND) --------------------
    const loadMoreBottom = useCallback(async () => {
        if (isLoadingMore) return;
        if (oldSales.length >= totalOldCount) return;

        const snap = captureAnchor();
        ...
        try {
            const result = await dispatch(
                cartSaleApi.endpoints.getFilteredSales.initiate(
                    {
                        page: nextPage,
                        limit: PAGE_LIMIT,
                        search: (searchQuery || "").trim()
                    },
                    { forceRefetch: false } // cache bo'lsa qayta urmaydi
                )
            ).unwrap();

            if (result?.innerData?.oldSales?.length) {
                setOldSales(prev => [...prev, ...result.innerData.oldSales]); // APPEND
                setOldPage(nextPage);

                // DOM yangilangandan keyin anchor restore
                requestAnimationFrame(() => restoreAnchor(snap));
            } else {
                setOldPage(nextPage);
                requestAnimationFrame(() => restoreAnchor(snap));
            }
    ...
    // OPTIONAL: oldingi page (REPLACE) — xohlasangiz qoldiring, bo‘lmasa olib tashlang
    const loadMoreTop = useCallback(async () => {
        ...
        try {
            ...
            if (result?.innerData?.oldSales) {
                setOldSales(result.innerData.oldSales); // REPLACE
    ...
    const handleReturnItemChange = useCallback((index, field, value) => {
        setReturnItems(prev => prev.map((item, i) => {
            if (i === index) {
                ...
                if (field === 'selected' && !value) updatedItem.returnQuantity = 0;
                return updatedItem;
            }
            return item;
        }));
    ...
    const getBalanceColor = (status) => (status === 'Qarzdor' ? 'hip-text-red' : 'hip-text-green');
    ...
    const openPaymentModal = useCallback((saleId) => {
        ...
        if (!sale) return toast.error("Sotuv topilmadi!");
    ...
    const openHistoryModal = (history) => {
        setModalState(prev => ({ ...prev, isHistoryModalOpen: true, activeHistory: history }));
    };
    ...
    const renderPopoverContent = useCallback((saleId) => (
        <div style={{ display: 'flex', alignItems: 'start', flexDirection: 'column', gap: '8px' }} className="renderPopover">
            <Button
                type="text"
                icon={<Edit size={16} />}
                onClick={(e) => { e.stopPropagation(); openEditModal(saleId); }}
            >
                Tahrirlash
            </Button>
            <Button
                type="text"
                icon={<Trash2 size={16} />}
                onClick={(e) => { e.stopPropagation(); processDeleteSale(saleId); }}
            >
                O‘chirish
            </Button>
            <Button
                ...
                onClick={(e) => { e.stopPropagation(); openReturnModal(saleId); }}
            >
                Vazvirat
            </Button>
        </div>
    ...
    const totalPages = Math.max(1, Math.ceil(totalOldCount / PAGE_LIMIT));

    // -------------------- LOADING UI RULES (FIX) --------------------
    // Table loader faqat birinchi load (data yo'q) bo‘lsa chiqadi.
    // Pagination bosilganda table o‘chmaydi, faqat button loading bo‘ladi.
    const showInitialLoader = initialLoading && !hasData;

    // -------------------- PAGINATION UI (ONE PLACE ONLY) --------------------
    const renderPaginationButtons = () => (
        ...
        >
            <Button
                ...
                onClick={(e) => { e.preventDefault(); e.stopPropagation(); loadMoreTop(); }}
            ...
            <span style={{ fontSize: '14px', color: '#666' }}>
                Sahifa {oldPage} / {totalPages}
            </span>

            <Button
                type="text"
                icon={<ChevronRight size={16} />}
                onClick={(e) => { e.preventDefault(); e.stopPropagation(); loadMoreBottom(); }}
                disabled={oldSales.length >= totalOldCount || isLoadingMore}
                loading={isLoadingMore && oldPage < totalPages}
                style={{ display: 'flex', alignItems: 'center' }}
            >
                Keyingiga
            </Button>
    ...
    return (
        <div className="hip-container">
                ...
                <table className="hip-table" aria-label="Sales Invoice Table">
                    ...
                    <tbody>
                        {showInitialLoader ? (
                            ...
                            filteredSalesData.map((customer, idxCustomer) => {
                                const rowExpanded = expandedRows.has(customer._id);

                                const totalHistoryAmount = customer.history
                                    ? customer.history.reduce((sum, h) => sum + (h.payment?.totalAmount || 0), 0)
                                    : 0;

                                return (
                                    <React.Fragment key={customer._id || idxCustomer}>
                                        <tr
                                            className="hip-tr"
                                        ...
                                        >
                                            ...
                                            <td className="hip-td">
                                                <div className="hip-td-box">
                                                    <button
                                                        ...
                                                        onClick={(e) => { e.stopPropagation(); openPaymentModal(customer._id); }}
                                                    >
                                                        To'lash
                                                    </button>
                                                    <button
                                                        type="primary"
                                                        disabled={(customer.Expenses?.length || 0) === 0}
                                                        onClick={(e) => { e.stopPropagation(); openHistoryModal(customer.Expenses); }}
                                            ...
                                            <td className="hip-td">
                                                <Button
                                                    ...
                                                    disabled={(customer.totalUndelivered || 0) === 0}
                                                    onClick={(e) => { e.stopPropagation(); openDeliveryModal(customer._id); }}
                                            ...
                                            <td className="hip-td">
                                                <div className="hip-td-box">
                                                    <button
                                                        type="primary"
                                                        disabled={!customer.groupedDeliveredItems || Object.keys(customer.groupedDeliveredItems).length === 0}
                                                        onClick={(e) => {
                                                            e.stopPropagation();
                                            ...
                                            <td className="hip-td">
                                                <div className="hip-balance-info">
                                                    ...
                                                    <span className="hip-history-count">
                                                        {customer.totalUndelivered || 0}
                                                        <TbTruckDelivery style={{ fontSize: "16px", marginLeft: "15px" }} />
                                                    </span>
                                                </div>
                                            </td>
                                        ...
                                        {rowExpanded && customer.history && (
                                            <>
                                                {customer.history.map((historyItem, idxHistory) => (
                                                    <tr key={historyItem._id || idxHistory} className="hip-history-row">
                                                        <td colSpan="10" className="hip-history-td">
                                                            <div className="hip-history-content">
                                                                <div className="hip-history-header">
                                                                    ...
                                                                    <div className="hip-payment-summary">
                                                                        <div className="hip-payment-item">
                                                                            ...
                                                                            <span className="hip-value">{formatCurrency(historyItem.payment?.totalAmount || 0)}</span>
                                                                        </div>

                                                                        <div className="hip-payment-item">
                                                                            <span className="hip-label">NDS (12%):</span>
                                                                            <span className="hip-value">{formatCurrency(historyItem.payment?.ndsTotal || 0)}</span>
                                                                        </div>

                                                                        {!!historyItem.payment?.middlemanPayment && historyItem.payment.middlemanPayment > 0 && (
                                                                            <div className="hip-payment-item">
                                                                                <span className="hip-label">Broker hizmati:</span>
                                                                                <span className="hip-value hip-text-red">
                                                                                    {formatCurrency(historyItem.payment.middlemanPayment)}
                                                                                </span>
                                                                            </div>
                                                                        )}
                                                                ...
                                                                <div className="hip-combined-section">
                                                                    ...
                                                                    {(historyItem.items || []).map((item, idxItem) => {
                                                                        const deliveredItem = historyItem.deliveredItems?.find(
                                                                            (delItem) => String(delItem.productId) === String(item.productId)
                                                                        ) || {};

                                                                        return (
                                                                            <div key={idxItem} className="hip-product-item">
                                                                                ...
                                                                                <div className="hip-detail">
                                                                                    ...
                                                                                    <span className="hip-value">{formatCurrency(item.sellingPrice || 0)}</span>
                                                                                </div>
            ...
            {modalState.isDeliveryModalOpen && (
                <DeliveryProduct closeModal={closeModal} modalState={modalState} />
            )}

            {modalState.isHistoryModalOpen && (
                <PaymentChecklist closeModal={closeModal} modalState={modalState} />
            )}

            {modalState.isDeliveryListOpen && (
                <DeliveryDisplayPrint closeModal={closeModal} modalState={modalState} />
            )}
