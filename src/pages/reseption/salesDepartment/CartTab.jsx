...
const CartTab = ({ cart = [], setCart, setActiveTab, onUpdateCart, onRemoveFromCart, onCompleteSale }) => {
    ...
    const removeFromCart = useCallback(
        (productId) => {
            ...
            setFocusedPrices(prev => { const newS = { ...prev }; delete newS[productId]; return newS; });
            setEditingPrices(prev => { const newS = { ...prev }; delete newS[productId]; return newS; });
        },
    ...
    const handlePriceFocus = useCallback((productId, basePrice) => {
        setFocusedPrices(prev => ({ ...prev, [productId]: true }));
        setEditingPrices(prev => ({ ...prev, [productId]: basePrice.toFixed(2) }));
    }, []);

    const handlePriceChange = useCallback((productId, e) => {
        let typed = e.target.value;
        // Clean: allow digits and one dot
        const cleaned = typed.replace(/[^0-9.]/g, '');
        // Ensure only one dot
        const dotIndex = cleaned.indexOf('.');
        let finalTyped = cleaned;
        if (dotIndex !== -1) {
            const integerPart = cleaned.substring(0, dotIndex);
            let decimalPart = cleaned.substring(dotIndex + 1);
            decimalPart = decimalPart.slice(0, 2); // Limit to 2 decimals
            finalTyped = integerPart + (decimalPart ? '.' + decimalPart : '');
        }
        setEditingPrices(prev => ({ ...prev, [productId]: finalTyped }));
        // Parse for real-time update
        const num = parseFloat(finalTyped) || 0;
        setContractInfo(prev => ({
            ...prev,
            discounts: { ...prev.discounts, [productId]: Math.max(num, 0) },
        }));
    }, []);

    const handlePriceBlur = useCallback((productId, e) => {
        setFocusedPrices(prev => {
            const newState = { ...prev };
            delete newState[productId];
            return newState;
        });
        const typed = e.target.value;
        // Final clean and parse
        let cleaned = typed.replace(/,/g, '').replace(/[^0-9.]/g, '');
        const parts = cleaned.split('.');
        let finalNum = 0;
        if (parts.length > 1) {
            const integer = parts[0] || '0';
            const decimal = (parts[1] || '00').slice(0, 2).padEnd(2, '0');
            finalNum = parseFloat(integer + '.' + decimal);
        } else {
            finalNum = parseFloat(cleaned) || 0;
        }
        setContractInfo(prev => ({
            ...prev,
            discounts: { ...prev.discounts, [productId]: Math.max(finalNum, 0) },
        }));
                    ...
                    <div className="card-cart-content">
                        <div className="card-cart-items">
                            {cart.map((item, index) => {
                                ...
                                const isFocused = focusedPrices[item._id] || false;
                                const editingValue = editingPrices[item._id] || basePrice.toFixed(2);
                                const displayValue = isFocused ? editingValue : formatNumber(basePrice);
                                const digitLength = displayValue.replace(/\D/g, '').length;
                                const inputWidth = getWidthByLength(digitLength);
                                            ...
                                            <div>
                                                ...
                                                <div className="card-cart-item-price">
                                                    <input
                                                        ...
                                                        inputMode="decimal"
                                                        value={displayValue}
                                                        onChange={(e) => handlePriceChange(item._id, e)}
                                                        onFocus={() => handlePriceFocus(item._id, basePrice)}
                                                        onBlur={(e) => handlePriceBlur(item._id, e)}
...
//     // Formatlangan qiymat (minglik va ikkita kasr)
//     const formatCurrency = (value) => {
//         if (!value) return "";

//         // Faqat raqam va nuqtani qoldiramiz
//         let cleaned = value.replace(/[^0-9.]/g, "");

//         // Nuqtadan keyin faqat 2 raqam qoldiramiz
//         if (cleaned.includes(".")) {
//             const [intPart, decimalPart] = cleaned.split(".");
//             cleaned = intPart + "." + decimalPart.slice(0, 2);
//         }

//         // Nuqta bor bo‘lsa, alohida qaytaramiz (kasr uchun)
//         if (cleaned.includes(".")) {
//             const [intPart, decimalPart] = cleaned.split(".");
//             return (
...
//         // Agar faqat butun son bo‘lsa
//         return Number(cleaned).toLocaleString("en-US");
//     };

//     const handleContractPriceChange = (productId, rawValue) => {
//         // Formatlangan qiymatni inputga qo‘yish
//         const formatted = formatCurrency(rawValue);

//         // Saqlash uchun toza raqam (float)
//         const numberValue = parseFloat(formatted.replace(/,/g, "")) || 0;

//         setContractInfo((prev) => ({
//             ...prev,
//             discounts: { ...prev.discounts, [productId]: numberValue },
//         }));
...
//                                                     <input
//                                                         type="text"
//                                                         value={formatCurrency(String(contractInfo.discounts[item._id] ?? item.sellingPrice ?? 0))}
//                                                         onChange={(e) => handleContractPriceChange(item._id, e.target.value)}
//                                                         className="card-price-input"
//                                                         style={{
//                                                             width: `${inputWidth + 5}px`,
