...
const PerformanceCell = React.memo(
  ({ actual, plan, unit = "", className = "" }) => {
      ...
      <td className={`sdash-data-cell ${className}`}>
        <div className="sdash-performance-wrapper">
          ...
          <div className={`sdash-performance-bar ${performanceClass}`}>
            <div
              className="sdash-progress-fill"
              style={{ width: `${Math.min(percentage, 100)}%` }}
            />
          </div>
          <div className={`sdash-percentage-text ${performanceClass}`}>
            {percentage}%
          </div>
        </div>
      </td>
...
const SalespersonDashboard = () => {
  ...
  const salesEmployees = employeesData?.innerData?.length
    ? role === "sotuvchi"
      ? employeesData?.innerData.filter(
          (employee) => employee?._id === workersId
        ) ?? []
  ...
  const allPlans = plansData?.innerData?.length
    ? role === "sotuvchi"
      ? plansData?.innerData.filter(
          (plan) => plan?.employeeId?._id === workersId
        ) ?? []
      : plansData?.innerData
    : [];
  ...
  const combineEmployeeData = () => {
    ...
    return salesEmployees?.map((employee) => {
      ...
      const currentPlan = employeePlans?.find(
        (plan) => plan.month === selectedMonth
      );
  ...
  const handleCreatePlan = async (e) => {
    ...
    if (
      !planForm.employeeId ||
      !planForm.targetAmount ||
      isNaN(planForm.targetAmount) ||
      planForm.targetAmount <= 0
    ) {
  ...
  const StatusBadge = ({ hasCurrentPlan }) => (
    <span
      className={`sdash-status-indicator ${
        hasCurrentPlan ? "sdash-status-active" : "sdash-status-inactive"
      }`}
    >
      {hasCurrentPlan ? (
        <>
  ...
  const calculateStatistics = () => {
    ...
    const activePlansCount = filteredEmployees.filter(
      (emp) => emp.currentPlan
    ).length;
    const totalAchieved = currentMonthPlans.reduce(
      (sum, plan) => sum + (plan.achievedAmount || 0),
      0
    );
  ...
  }, [filteredEmployees, allPlans, selectedMonth]);
        ...
        <div className="sdash-header-content">
          <div className="sdash-header-left">
            ...
            <div className="sdash-filter-controls">
              ...
              <input
                type="text"
                placeholder="Sotuvchini qidiring..."
                value={searchTerm}
                onChange={(e) => setSearchTerm(e.target.value)}
                className="sdash-search-input"
              />
            </div>
          </div>
          <div className="sdash-statistics-grid">
            <div className="sdash-stat-card">
              <div className="sdash-stat-number">
                {statistics.activePlansCount}
              </div>
              <div className="sdash-stat-text">Faol planlar</div>
            </div>
            <div className="sdash-stat-card">
              <div className="sdash-stat-number">
                {statistics.currentMonthPlansCount}
              </div>
                  ...
                  <tr key={employee._id || index} className="sdash-data-row">
                    ...
                    <td className="sdash-data-cell">
                      {capitalizeFirstLetter
                        ? capitalizeFirstLetter(employee.unit || "sotuvchi")
                        : employee.unit || "Sotuvchi"}
                    </td>
                    <td className="sdash-data-cell">
                      ...
                      <div className="sdash-action-group">
                        {employee.currentPlan && role !== "sotuvchi" ? (
                          <>
                            <button
                              onClick={() =>
                                openEditPlanModal(employee.currentPlan)
                              }
                            ...
                            <button
                              onClick={() =>
                                handleDeletePlan(employee.currentPlan._id)
                              }
                          ...
                          <>
                            {role !== "sotuvchi" && (
                              <button
                                onClick={() =>
                                  openCreatePlanModal(employee._id)
                                }
              ...
              >
                <div className="form-group">
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setPlanForm({ ...planForm, employeeId: e.target.value })
                    }
                ...
                <div className="form-group">
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setPlanForm({ ...planForm, month: e.target.value })
                    }
                    className="form-select"
                    aria-label="Oyni tanlash"
                  >
                ...
                <div className="summary-cards">
                  <div className="summary-card">
                    <div className="summary-value">
                      {selectedEmployee.allPlans?.length || 0}
                    </div>
                    <div className="summary-label">Jami planlar</div>
                  </div>
                  <div className="summary-card">
                    <div className="summary-value">
                      {selectedEmployee.currentPlan?.achievedAmount?.toLocaleString() ||
                        0}
                    </div>
                  ...
                  <div className="summary-card">
                    <div className="summary-value">
                      ...
                      selectedEmployee.currentPlan.targetAmount > 0
                        ? Math.round(
                            (selectedEmployee.currentPlan.achievedAmount /
                              selectedEmployee.currentPlan.targetAmount) *
                              100
                          )
                        : 0}
                      %
                      ...
                      <div key={plan._id} className="plan-card">
                        <div className="plan-header">
                          ...
                          <div
                            className={`plan-status ${
                              (plan.achievedAmount || 0) >= plan.targetAmount
                                ? "completed"
                                : "pending"
                            }`}
                          >
                        ...
                        <div className="plan-details">
                          <div className="plan-amounts">
                            <span>
                              Plan: {plan.targetAmount.toLocaleString()} so'm
                            </span>
                            <span>
                              Bajarilgan:{" "}
                              {(plan.achievedAmount || 0).toLocaleString()} so'm
                            </span>
                          </div>
...
//                             return (
//                                 <tr key={inx} className="sdash-data-row">
//                                     <td className="sdash-data-cell">
//                                         <div className="sdash-employee-wrapper">
//                                             <div className="sdash-employee-avatar">
//                                                 <User className="sdash-avatar-icon" />
